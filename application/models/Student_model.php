...
class Student_model extends MY_Model
{
    ...
    public function save($data = array(), $getBranch = array())
    {
        ...
        $inser_data1 = array(
            ...
            'branch_id' => $this->application_model->get_branch_id()  // <-- CRITICAL FIX
        );
    ...
    public function csvImport($row = array(), $classID = '', $sectionID = '', $branchID = '', $sessionID = '')
{
    // getting existing parent data by username
    $getParent = array();
    if (!empty($row['GuardianUsername'])) {
        $getParent = $this->db->select('parent.id')
            ->from('login_credential')
            ->join('parent', 'parent.id = login_credential.user_id', 'left')
            ->where(array(
                'parent.branch_id' => $branchID, 
                'login_credential.username' => $row['GuardianUsername']
            ))
            ->get()
            ->row_array();
    }
    ...
    if (isset($getParent) && !empty($getParent)) {
    ...
    // Prepare student data with proper null checks
    $inser_data1 = array(
        ...
        'category_id' => !empty($row['CategoryID']) ? $row['CategoryID'] : 0,
        'admission_date' => !empty($row['AdmissionDate']) ? date("Y-m-d", strtotime($row['AdmissionDate'])) : date('Y-m-d'),
    ...
    $inser_data1 = array_filter($inser_data1, function($value) {
    ...
    // Use the sessionID passed from controller, fallback to default only if not provided
    $finalSessionID = !empty($sessionID) ? $sessionID : get_session_id();
    ...
    $arrayEnroll = array(
        ...
        'branch_id' => $branchID,
        'roll' => !empty($row['Roll']) ? $row['Roll'] : 0,
        'session_id' => $finalSessionID,  // CRITICAL FIX: use passed session ID
    );
