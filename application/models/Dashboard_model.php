...
class Dashboard_model extends CI_Model
{
    ...
    /**
 * Get TIS chart data for dashboard
 */
public function get_tis_chart_data($branch_id = null, $period = 'month')
{
    if (!$branch_id) $branch_id = get_loggedin_branch_id();
    
    // Get performance trend over time
    $sql = "SELECT DATE(calculated_at) as date, AVG(total_score) as avg_score
    ...
    $trend = $this->db->query($sql, [$branch_id])->result_array();
    
    // Get top performing teachers
    $top_teachers = $this->db->select('s.name, tps.total_score, tps.rating')
                             ->from('teacher_performance_scores tps')
                             ->join('staff s', 's.id = tps.teacher_id')
                             ->where('tps.branch_id', $branch_id)
                             ->where('tps.session_id', get_session_id())
    ...
    $at_risk = $this->db->select('s.name, trp.risk_level, trp.prediction_score, trp.recommended_action')
                        ->from('teacher_risk_predictions trp')
                        ->join('staff s', 's.id = trp.teacher_id')
                        ->where('trp.branch_id', $branch_id)
                        ->where('trp.risk_level !=', 'low')
                        ->where('trp.session_id', get_session_id())
                        ->order_by('trp.prediction_score', 'DESC')
                        ->limit(5)
                        ->get()
                        ->result_array();
