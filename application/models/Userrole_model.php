 ...
 */
public function getHomeworkList($student_id)
{
    // Get student's class and section from enrollment
    $enroll = $this->db->select('class_id, section_id, session_id, branch_id')
    ...
    // Get homework for this class/section
    $this->db->select('h.*, sub.name as subject_name, c.name as class_name, sec.name as section_name,
                      hs.id as submission_id, hs.submission_status, hs.feedback, hs.grade, hs.created_at as submitted_at,
             ...
             ->join('homework_evaluation he', 'he.homework_id = h.id AND he.student_id = ' . $this->db->escape($student_id), 'left')
             ->where('h.class_id', $enroll['class_id'])
             ->where('h.section_id', $enroll['section_id'])
             ->where('h.branch_id', $enroll['branch_id'])
   ...
   public function getOfflinePaymentsList()
    {
        $student_enroll_id = $this->session->userdata('enroll_id'); // or however you store it
        ...
        $sql = "SELECT 
                ...
                WHERE op.student_enroll_id = " . $this->db->escape($student_enroll_id) . "
                ORDER BY op.id DESC";
    ...
    /**
     * Get student submissions (submitted homework)
     */
    public function getStudentSubmissions($student_id)
    {
        $this->db->select('hs.*, h.subject_id, h.class_id, h.section_id, h.date_of_homework, h.date_of_submission,
                 ...
                 ->join('class c', 'c.id = h.class_id')
                 ->join('section sec', 'sec.id = h.section_id')
                 ->join('homework_evaluation he', 'he.homework_id = h.id AND he.student_id = hs.student_id', 'left')
                 ->where('hs.student_id', $student_id)
                 ->order_by('hs.created_at', 'DESC');
        
        return $this->db->get()->result_array();
    }
    
/**
 * Get graded homework (with feedback from homework_evaluation table)
 */
public function getStudentGradedHomework($student_id)
{
    ...
    // Get homework that has been evaluated (has entry in homework_evaluation)
    $this->db->select('h.*, sub.name as subject_name, c.name as class_name, sec.name as section_name,
                      hs.id as submission_id, hs.message as submission_message, hs.enc_name, hs.file_name,
                      hs.created_at as submitted_at,
                      he.rank as grade, he.remark as feedback, he.date as feedback_date, he.status as evaluation_status')
             ->from('homework h')
             ->join('subject sub', 'sub.id = h.subject_id')
             ->join('class c', 'c.id = h.class_id')
             ->join('section sec', 'sec.id = h.section_id')
             ->join('homework_submit hs', 'hs.homework_id = h.id AND hs.student_id = ' . $this->db->escape($student_id), 'left')
             ->join('homework_evaluation he', 'he.homework_id = h.id AND he.student_id = ' . $this->db->escape($student_id), 'inner')
             ->where('h.class_id', $enroll['class_id'])
             ->where('h.section_id', $enroll['section_id'])
             ->where('h.branch_id', $enroll['branch_id'])
             ->where('h.session_id', get_session_id())
             ->where('he.rank IS NOT NULL')  // Grade exists
             ->order_by('he.date', 'DESC');
 ...
 */
public function getStudentHomeworkWithStatus($student_id)
{
    $enroll = $this->db->select('class_id, section_id, session_id, branch_id')
                       ->where('student_id', $student_id)
                       ->where('session_id', get_session_id())
...
public function getMyChildren()
{
    ...
    $this->db->select('s.id as student_id, CONCAT(s.first_name, " ", s.last_name) as name, 
                      c.name as class_name, sec.name as section_name, s.register_no')
             ->from('student s')
