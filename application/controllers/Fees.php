...
class Fees extends Admin_Controller
{
    ...
    // In Fees.php, replace the current fee_add() method with this version:
...
public function fee_add()
{
    ...
    if ($this->form_validation->run() !== false) {
        ...
        // Calculate balance for SMS
        $balance = 0;
        if (!empty($feesType[0]) && !empty($feesType[1])) {
            $b = $this->fees_model->getBalance($feesType[0], $feesType[1]);
            if (isset($b['balance'])) {
                $original_balance = $b['balance'];
                $balance = $original_balance - ($amount - $discountAmount);
                if ($balance < 0) $balance = 0;
                
                // log_message('debug', "Balance calculation - Original: {$original_balance}, Amount: {$amount}, Discount: {$discountAmount}, New Balance: {$balance}");
            }
        }
        
        $total_paid = ($amount + $fineAmount) - $discountAmount;
        
        // ========== CRITICAL: SMS CREDIT CHECK AND SENDING BEFORE SAVING ==========
        $sms_required = isset($_POST['guardian_sms']);
        $sms_result = false;
        $sms_message = '';
        $sms_credits_checked = false;
        ...
        if ($sms_required) {
            ...
            // Get SMS template for fees (ID 2)
            ...
            if (!empty($template) && !empty($template['template_body'])) {
                // Count recipients based on template settings
                $recipient_count = 0;
                $student = $this->application_model->getstudentdetails($student_id);
                ...
                if (!empty($student)) {
                    ...
                    if ($template['notify_parent'] == 1 && !empty($student['parent_id'])) {
                        $parent = $this->db->select('mobileno')
                ...
                if ($recipient_count > 0) {
                    ...
                    $credits_needed = $this->calculate_sms_cost_fees($template['template_body'], $recipient_count);
                    
                    // Check available credits
                    $current_credits = $this->sendsmsmail_model->get_sms_credit($branch_id);
                    $sms_credits_checked = true;
                    
                    // log_message('debug', "SMS Credits Check - Needed: {$credits_needed}, Available: {$current_credits}, Recipients: {$recipient_count}");
                    
                    if ($current_credits >= $credits_needed) {
                        // Prepare SMS data with balance
                        $sms_data = [
                        ...
                        // Try to send SMS (this will check credits internally and send if possible)
                        $sms_result = $this->sms_model->send_sms($sms_data, 2);
                        
                        if ($sms_result) {
                            $sms_message = 'SMS sent successfully';
                            // log_message('info', 'Fees SMS sent successfully for student ID: ' . $student_id . ' with balance: ' . $balance);
                        } else {
                            $sms_message = 'SMS failed to send (gateway error or insufficient credits)';
                            // log_message('error', 'Fees SMS failed for student ID: ' . $student_id);
                        }
                    } else {
                        // INSUFFICIENT CREDITS - DO NOT SAVE
                        $sms_message = 'Insufficient SMS credits. Needed: ' . $credits_needed . ', Available: ' . $current_credits;
                        // log_message('error', 'Fees SMS blocked - ' . $sms_message);
                        
                        // Return error response
                        $array = array(
                            'status' => 'fail', 
                            'url' => '', 
                            'error' => array('guardian_sms' => $sms_message)
                        );
                        echo json_encode($array);
                        return;
                    }
                } else {
                    $sms_message = 'No recipients found for SMS notification';
                    // log_message('warning', $sms_message);
                }
            } else {
                $sms_message = 'SMS template not found or empty';
                // log_message('error', $sms_message);
            }
        }
        // ========== END SMS CREDIT CHECK ==========
        
        // Start database transaction to ensure atomic operation
        $this->db->trans_start();
        
        try {
            // Save payment record (only if SMS check passed or not required)
            $arrayFees = array(
                'allocation_id' => $feesType[0],
                'type_id' => $feesType[1],
            ...
            if (isset($_POST['account_id'])) {
                ...
                // log_message('debug', "Transaction saved - Account: {$account_id}, Amount: {$total_paid}");
            }
            ...
            // log_message('info', "Payment saved successfully - Student: {$student_id}, Amount: {$total_paid}, Payment ID: {$payment_historyID}");

            // Prepare AJAX response with SMS status
            $response_data = [
                'status' => 'success',
                'message' => translate('information_has_been_saved_successfully'),
            ...
            // Add SMS status if SMS was attempted
            if ($sms_required) {
                $response_data['sms_status'] = [
                    'success' => $sms_result,
                    'message' => $sms_message,
                    'credits_checked' => $sms_credits_checked
                ];
            }
            ...
            if ($this->input->is_ajax_request()) {
                ...
                redirect(base_url('fees/invoice/' . $student_id));
            }
            ...
            $error_message = 'Failed to save payment. Please try again.';
            if ($this->input->is_ajax_request()) {
                $array = array(
                    'status' => 'fail', 
                    'url' => '', 
                    'error' => array('general' => $error_message)
                );
                echo json_encode($array);
            } else {
 ...
 public function fee_fully_paid()
{
    ...
    if ($this->form_validation->run() !== false) {
        ...
        $total_paid = $totalBalance + $totalFine;
        ...
        if ($sms_required) {
            ...
            if (!empty($template) && !empty($template['template_body'])) {
                ...
                if ($recipient_count > 0) {
                    ...
                    // log_message('debug', "Fully Paid SMS Credits Check - Needed: {$credits_needed}, Available: {$current_credits}, Recipients: {$recipient_count}");
                    
                    if ($current_credits >= $credits_needed) {
                        // Prepare SMS data with balance (balance will be 0 for fully paid)
                        $sms_data = [
                            'student_id' => $student_id,
                            'amount' => $total_paid,
                            'balance' => 0, // Fully paid means balance is 0
                            'paid_date' => _d($date),
                            'full_payment' => true
                        ];
                        ...
                        if ($sms_result) {
                            $sms_message = 'SMS sent successfully';
                            // log_message('info', 'Fully paid SMS sent successfully for student ID: ' . $student_id);
                        } else {
                            $sms_message = 'SMS failed to send (gateway error or insufficient credits)';
                            // log_message('error', 'Fully paid SMS failed for student ID: ' . $student_id);
                        }
                    } else {
                        // INSUFFICIENT CREDITS - DO NOT SAVE
                        $sms_message = 'Insufficient SMS credits. Needed: ' . $credits_needed . ', Available: ' . $current_credits;
                        // log_message('error', 'Fully paid SMS blocked - ' . $sms_message);
        ...
        try {
            ...
            if (isset($_POST['account_id'])) {
                ...
                // log_message('debug', "Fully paid transaction saved - Account: {$account_id}, Amount: {$total_paid}");
            }
            ...
            // log_message('info', "Fully paid saved successfully - Student: {$student_id}, Total: {$total_paid}");
            ...
            // log_message('error', 'Fully paid save failed: ' . $e->getMessage());
