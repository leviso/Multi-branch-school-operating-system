 ...
 public function getSubjectByClass()
{
    ...
    // Validate branch_id - must be provided, no fallback
    if (empty($branch_id)) {
        echo '<option value="">Select Branch First</option>';
        return;
    }
    
    // Direct query using prepared statement
    ...
    $query = $this->db->query($sql, array($class_id, $branch_id));
    ...
    if (!empty($subjects)) {
        foreach ($subjects as $subject) {
            $html .= '<option value="' . $subject->id . '">' . htmlspecialchars($subject->name, ENT_QUOTES, 'UTF-8') . '</option>';
        }
    } else {
