...
{
    // Only superadmin can access payment settings
    ...
    // ✅ FIXED: Get existing config from database
    $this->db->where('branch_id', $branchID);
    $config = $this->db->get('payment_config')->row_array();
  ...
  public function bulksmsbd()
{
    ...
    $this->form_validation->set_rules('sender_id', translate('sender_id'), 'trim|required');
    $this->form_validation->set_rules('api_key', translate('api_key'), 'trim|required');
    $this->form_validation->set_rules('shortcode', translate('shortcode'), 'trim|required');
    
    if ($this->form_validation->run() !== false) {
        $arraySMScountry = array(
            'field_one' => $this->input->post('sender_id'),
            'field_two' => $this->input->post('api_key'),
            'field_three' => $this->input->post('shortcode'), // ADD THIS LINE
        );
    ...
    /**
 * Save M-Pesa STK Push configuration
 */
public function mpesa_stk_save()
{
    // Only superadmin can access
    if (!is_superadmin_loggedin()) {
        echo json_encode(array('status' => 'fail', 'message' => 'Access denied. Superadmin only.'));
        return;
    }
    
    // ✅ FIXED: Get branch ID correctly
    ...
    // log_message('info', 'M-Pesa STK Save - Branch ID: ' . $branchID);
    ...
    $this->form_validation->set_rules('shortcode', 'Shortcode', 'trim|required');
    $this->form_validation->set_rules('passkey', 'Passkey', 'trim|required');
    
    if ($this->form_validation->run() == true) {
        $environment = isset($_POST['environment']) ? 'sandbox' : 'production';
        $mpesa_stk_status = isset($_POST['mpesa_stk_status']) ? 1 : 0;
        
        // Check if record exists for this branch
        $this->db->where('branch_id', $branchID);
        $existing = $this->db->get('payment_config')->row();
        ...
        if ($existing) {
            ...
            $update_data = array(
                ...
                'mpesa_stk_status' => $mpesa_stk_status,
                'updated_at' => date('Y-m-d H:i:s')
            );
            
            $this->db->where('branch_id', $branchID);
            $result = $this->db->update('payment_config', $update_data);
            
            if ($result) {
                echo json_encode(array('status' => 'success', 'message' => 'M-Pesa STK configuration saved successfully'));
            } else {
                echo json_encode(array('status' => 'fail', 'message' => 'Failed to update configuration.'));
            }
        } else {
            // Insert new record
            $insert_data = array(
            ...
            $result = $this->db->insert('payment_config', $insert_data);
            
            if ($result) {
                echo json_encode(array('status' => 'success', 'message' => 'M-Pesa STK configuration saved successfully'));
            } else {
                echo json_encode(array('status' => 'fail', 'message' => 'Failed to insert configuration.'));
            }
        }
    } else {
        $error = $this->form_validation->error_array();
...
/**
 * Save M-Pesa Paybill (C2B) configuration
 */
public function mpesa_paybill_save()
{
    // Only superadmin can access
    if (!is_superadmin_loggedin()) {
        echo json_encode(array('status' => 'fail', 'message' => 'Access denied. Superadmin only.'));
    ...
    $this->form_validation->set_rules('paybill_number', 'Paybill Number', 'trim|required');
    
    if ($this->form_validation->run() == true) {
        $mpesa_paybill_status = isset($_POST['mpesa_paybill_status']) ? 1 : 0;
        ...
        if ($existing) {
            $update_data = array(
                ...
                'mpesa_paybill_status' => $mpesa_paybill_status,
                'updated_at' => date('Y-m-d H:i:s')
            );
        ...
        if ($result) {
            echo json_encode(array('status' => 'success', 'message' => 'M-Pesa Paybill configuration saved successfully'));
        } else {
            echo json_encode(array('status' => 'fail', 'message' => 'Database error: Could not save configuration'));
        }
    } else {
        $error = $this->form_validation->error_array();
        echo json_encode(array('status' => 'fail', 'error' => $error));
...
public function pesapal_save()
{
    ...
    $this->form_validation->set_rules('pesapal_consumer_key', 'Consumer Key', 'trim|required');
    $this->form_validation->set_rules('pesapal_consumer_secret', 'Consumer Secret', 'trim|required');
    
    if ($this->form_validation->run() == true) {
        $pesapal_sandbox = isset($_POST['pesapal_sandbox']) ? 1 : 0;
        $pesapal_status = isset($_POST['pesapal_status']) ? 1 : 0;
        ...
        if ($existing) {
            $update_data = array(
                'pesapal_consumer_key' => $this->input->post('pesapal_consumer_key'),
                'pesapal_consumer_secret' => $this->input->post('pesapal_consumer_secret'),
                'pesapal_sandbox' => $pesapal_sandbox,
        ...
        if ($result) {
            echo json_encode(array('status' => 'success', 'message' => 'Pesapal configuration saved successfully'));
        } else {
            echo json_encode(array('status' => 'fail', 'message' => 'Database error: Could not save configuration'));
        }
    } else {
