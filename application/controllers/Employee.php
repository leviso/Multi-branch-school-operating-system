...
public function workload() {
    ...
    if (!$branch_id) {
        if (is_superadmin_loggedin()) {
            // Show branch selector for superadmin
    ...
    $data['workload_distribution'] = $this->employee_model->get_workload_distribution($branch_id, $session_id);
    
    // Get workload data for each teacher
    $data['workload_data'] = array();
    foreach ($data['teachers'] as $teacher) {
        $data['workload_data'][$teacher->id] = $this->employee_model->get_teacher_workload($teacher->id, $session_id);
    }
...
public function performance() {
    ...
    $session_id = get_session_id();
    $term_id = $this->input->get('term_id') ?: $this->employee_model->get_current_term_id($branch_id);
    ...
    $data['main_menu'] = 'tis';
    $data['branch_id'] = $branch_id;
    $data['selected_term'] = $term_id;
    $data['show_branch_selector'] = false;
    
    // Get all data from model
    $data['teachers'] = $this->employee_model->getStaffList($branch_id, 3, 1);
    $data['performance_data'] = $this->employee_model->get_performance_data($branch_id, $session_id, $term_id, $rating_filter);
    $data['stats'] = $this->employee_model->get_performance_distribution($branch_id, $session_id, $term_id);
    $data['top_performers'] = $this->employee_model->get_top_performers($branch_id, $session_id, $term_id, 5);
...
public function risks() {
    ...
    $session_id = get_session_id();
    $term_id = $this->employee_model->get_current_term_id($branch_id);
    ...
    // Get all data from model
    $data['teachers'] = $this->employee_model->getStaffList($branch_id, 3, 1);
    $data['risk_distribution'] = $this->employee_model->get_risk_distribution($branch_id, $session_id, $term_id);
    $data['risk_data'] = $this->employee_model->get_all_teacher_risks($branch_id, $session_id, $term_id, $data['teachers']);
    $data['high_risk_teachers'] = $this->employee_model->get_high_risk_teachers($branch_id, $session_id, $term_id);
 ...
 * My Duties View
 */
public function my_duties() {
    if (!get_permission('tis_duties', 'is_view')) {
        access_denied();
    }
    
    // For duties, we use the logged-in user's branch
    $branch_id = get_loggedin_branch_id();
    
    if (!$branch_id) {
        if (is_superadmin_loggedin()) {
            // Superadmin viewing duties - need to select branch
            $branch_id = $this->input->get('branch_id');
            if (!$branch_id) {
    ...
    // Get all data from model
    $data['duties'] = $this->employee_model->get_teacher_duties($teacher_id, $branch_id, $session_id);
    $data['stats'] = $this->employee_model->get_duty_statistics($teacher_id, $branch_id, $session_id);
    $data['total_points'] = $this->employee_model->get_total_duty_points($teacher_id, $branch_id, $session_id);
 ...
 */
private function generate_workload_recommendations($teachers, $distribution) {
    $recommendations = [];
    
    // Overall balance recommendations
    if ($distribution->overloaded_count > $distribution->underutilized_count * 2) {
        $recommendations[] = [
            'type' => 'balance',
            'message' => 'Workload is heavily skewed. Consider redistributing tasks from overloaded to underutilized teachers.'
        ];
    }
    
    // Individual teacher recommendations
    foreach ($teachers as $teacher) {
        if ($teacher->workload && $teacher->workload->status == 'overloaded') {
            $recommendations[] = [
                'type' => 'reassign',
                'teacher_id' => $teacher->id,
                'teacher_name' => $teacher->name,
                'message' => "{$teacher->name} is overloaded ({$teacher->workload->workload_score}%). Consider reducing load by " . round($teacher->workload->workload_score - 100) . "%."
            ];
        } elseif ($teacher->workload && $teacher->workload->status == 'underutilized') {
            $recommendations[] = [
                'type' => 'assign',
                'teacher_id' => $teacher->id,
                'teacher_name' => $teacher->name,
                'message' => "{$teacher->name} is underutilized. Can take on " . round(100 - $teacher->workload->workload_score) . "% more workload."
            ];
        }
    }
