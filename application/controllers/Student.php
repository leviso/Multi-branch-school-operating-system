...
class Student extends Admin_Controller
{
 ...
 public function add()
{
    // ========== STARS: Preserve source parameter ==========
    $stars_source = $this->input->get('source');
    $stars_branch_id = $this->input->get('branch_id');
    
    // If branch filter was submitted with source, redirect to preserve it
    if ($this->input->post('search') && $this->input->post('source')) {
        $branch_id = $this->input->post('branch_id');
        $class_id = $this->input->post('class_id');
        $source = $this->input->post('source');
        redirect(base_url("student/add?branch_id={$branch_id}&class_id={$class_id}&source={$source}"));
        return;
    }
    
    // Override branch_id if from URL (for superadmin)
    if (!empty($stars_branch_id) && is_superadmin_loggedin()) {
        $_POST['branch_id'] = $stars_branch_id;
    }
    // ========== END STARS ==========
   ...
   public function save() {
    if ($_POST) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            // ========== STARS TRANSFER HANDLING ==========
            $is_transfer = $this->input->post('is_transfer_student');
            ...
            if ($is_transfer == 1) {
                ...
                // Get current term
                $current_term = $this->stars_model->get_current_term_id($branchID);
                
                // Create transfer assessment record
                $transfer_data = array(
                    'student_id' => $studentID,
                    'branch_id' => $branchID,
                    'admission_id' => null,
                    'previous_school_name' => $this->input->post('previous_school') ?? null,
                    'transfer_date' => $this->input->post('transfer_date') ?? date('Y-m-d'),
                    'joining_class_id' => $post['class_id'],
                    'joining_section_id' => $post['section_id'] ?? null,
                    'joining_term_id' => $current_term,
                    'status' => 'pending',
                    'is_manual_admission' => 1,
                    'created_at' => date('Y-m-d H:i:s')
                );
                
                $this->db->insert('transfer_assessments', $transfer_data);
                $transfer_id = $this->db->insert_id();
                
                // Redirect to STARS assessment
                set_alert('success', 'Student added. Now complete the transfer assessment.');
                $array = array(
                    'status' => 'success', 
                    'url' => base_url('stars/assessment_detail/' . $transfer_id)
                );
                echo json_encode($array);
                return;
            }
            // ========== END STARS TRANSFER HANDLING ==========
...
public function csv_import()
{
    ...
    if (!get_permission('multiple_import', 'is_add')) {
        if ($is_ajax) {
            $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => translate('access_denied'))));
            return;
        }
        access_denied();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $this->form_validation->set_rules('year_id', 'Academic Year', 'trim|required');
        ...
        if (!isset($_FILES['userfile']) || $_FILES['userfile']['error'] != UPLOAD_ERR_OK) {
            if ($is_ajax) {
                $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => 'Please select a valid CSV file')));
                return;
            }
        ...
        if ($this->form_validation->run() == true) {
            ...
            $sessionID = $this->input->post('year_id');
            ...
            if ($csv_array && count($csv_array) > 0) {
                ...
                $missingHeaders = array_diff($columnHeaders, $csvHeaders);
                ...
                if (count($missingHeaders) <= 0) {
                    ...
                    foreach ($csv_array as $index => $row) {
                        ...
                        if ($index == 0 && (strtolower($row['FirstName']) == 'firstname' || empty($row['FirstName']))) {
                            continue;
                        }
                        
                        // Skip empty rows
                        if (empty($row['FirstName']) && empty($row['RegisterNo'])) {
                            continue;
                        }
                        
                        // Check required fields
                        if (empty($row['FirstName']) || empty($row['RegisterNo']) || empty($row['StudentUsername'])) {
                            $err_msg .= "Row " . ($index + 1) . " - Missing required fields (FirstName, RegisterNo, StudentUsername)<br>";
                            continue;
                        }
                        
                        $r = $this->csvCheckExistsData($row['StudentUsername'], $row['Roll'], $row['RegisterNo'], $classID, $sectionID, $branchID, $unique_roll, $sessionID);
                        
                        if ($r['status'] == false) {
                            $err_msg .= $row['FirstName'] . ' ' . $row['LastName'] . " - Import Failed : " . $r['message'] . "<br>";
                        } else {
                            $this->student_model->csvImport($row, $classID, $sectionID, $branchID, $sessionID);
                            $i++;
                        }
                    }
                    ...
                    if ($is_ajax) {
                        if ($err_msg != '') {
                            $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'partial', 'message' => $i . ' Students Added Successfully.<br><br>Errors:<br>' . $err_msg)));
                        } elseif ($i > 0) {
                            $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'success', 'message' => $i . ' Students Have Been Successfully Added')));
                        } else {
                            $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => 'No students were imported. ' . $err_msg)));
                        }
                        return;
                    }
                } else {
                    if ($is_ajax) {
                        $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => 'Invalid CSV format. Missing columns: ' . implode(', ', $missingHeaders))));
                        return;
                    }
                ...
                if ($is_ajax) {
                    $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => 'Could not read CSV file. Please check file format.')));
                    return;
                }
                set_alert('error', translate('invalid_csv_file'));
                redirect(base_url("student/csv_import"));
            ...
            if ($is_ajax) {
                ...
                $first_error = !empty($errors) ? reset($errors) : 'Validation failed';
                $this->output->set_content_type('application/json')->set_output(json_encode(array('status' => 'fail', 'message' => $first_error)));
                return;
            }
        }
    }
    
    // If not POST, load the view
    if (!$is_ajax) {
...
public function delete_data($eid = '', $sid = '')
{
    if (get_permission('student', 'is_delete')) {
        ...
        try {
            ...
            // 1. Delete from hostel_item_assignments (child of student)
            $this->db->where('student_id', $sid)->delete('hostel_item_assignments');
            
            // 2. Update hostel_inventory_items (set student_id to NULL, not delete)
            ...
            // 3. Delete student attendance (uses enroll_id)
            $this->db->where('enroll_id', $eid)->delete('student_attendance');
            
            // 4. Delete homework evaluation
            $this->db->where('student_id', $sid)->delete('homework_evaluation');
            
            // 5. Delete homework submissions
            $this->db->where('student_id', $sid)->delete('homework_submit');
            
            // 6. Delete online exam answers
            $this->db->where('student_id', $sid)->delete('online_exam_answer');
            
            // 7. Delete online exam attempts
            $this->db->where('student_id', $sid)->delete('online_exam_attempts');
            
            // 8. Delete online exam submissions
            $this->db->where('student_id', $sid)->delete('online_exam_submitted');
            
            // 9. Delete exam marks
            $this->db->where('student_id', $sid)->delete('mark');
            
            // 10. Delete exam attendance
            $this->db->where('student_id', $sid)->delete('exam_attendance');
            
            // 11. Delete transport assignments
            $this->db->where('student_id', $sid)->delete('transport');
            
            // 12. Delete student trip bookings
            $this->db->where('student_id', $sid)->delete('student_trip_bookings');
            
            // 13. Delete messages
            $this->db->group_start();
            $this->db->where('sender', $sid);
            $this->db->or_where('reciever', $sid);
            $this->db->group_end();
            $this->db->delete('message');
            
            // 14. Delete message replies
            $this->db->where('identity', $sid)->delete('message_reply');
            
            // 15. Delete student documents and files
            $documents = $this->db->select('enc_name')->where('student_id', $sid)->get('student_documents')->result_array();
            foreach ($documents as $doc) {
                $file_path = FCPATH . 'uploads/attachments/documents/' . $doc['enc_name'];
                if (file_exists($file_path)) {
                    @unlink($file_path);
                }
            }
            $this->db->where('student_id', $sid)->delete('student_documents');
            
            // 16. Delete fee allocations and payment history
            $fee_allocations = $this->db->select('id')->where('student_id', $sid)->get('fee_allocation')->result_array();
            $fee_ids = array_column($fee_allocations, 'id');
            if (!empty($fee_ids)) {
                $this->db->where_in('allocation_id', $fee_ids)->delete('fee_payment_history');
            }
            $this->db->where('student_id', $sid)->delete('fee_allocation');
            
            // 17. Delete offline fees payments
            $this->db->where('student_id', $sid)->delete('offline_fees_payments');
            
            // 18. Delete from online admission interviews
            $this->db->where('student_id', $sid)->delete('online_admission_interviews');
            
            // 19. Delete custom fields values
            ...
            // 20. Delete student login credential
            $this->db->where(array('user_id' => $sid, 'role' => 7))->delete('login_credential');
            
            // 21. Delete from enroll table (this will cascade to some tables if set up)
            $this->db->where('id', $eid)->delete('enroll');
            
            // 22. Finally, delete the student record
            ...
            if ($this->db->trans_status() === FALSE) {
                $error = $this->db->error();
                log_message('error', 'Student deletion failed: ' . print_r($error, true));
                set_alert('error', 'Failed to delete student record: ' . $error['message']);
            } else {
            ...
            log_message('error', 'Exception in delete_data: ' . $e->getMessage());
            set_alert('error', 'Failed to delete student: ' . $e->getMessage());
        }
    }
}
    ...
    public function csvCheckExistsData($student_username = '', $roll = '', $registerno = '', $class_id = '', $section_id = '', $branchID = '', $unique_roll, $session_id = '')
{
    $array = array();
