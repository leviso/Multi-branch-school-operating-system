...
class Sessions extends Admin_Controller
{
    ...
    public function set_academic($action = '')
    {
        if (is_loggedin()) {
            ...
            // Update global settings if user is superadmin
            if (is_superadmin_loggedin()) {
                $this->db->where('id', 1);
                $this->db->update('global_settings', array('session_id' => $action));
            }
    ...
    public function delete($id = '')
    {
        if (is_superadmin_loggedin())
        {
            // Don't allow deletion of currently active global session
            $global_session = $this->db->get_where('global_settings', array('id' => 1))->row()->session_id;
