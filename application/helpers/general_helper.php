...
function get_session_id()
{
    ...
    if ($CI->session->has_userdata('set_session_id')) {
        ...
        // Use raw query to avoid any CI query builder issues
        ...
        if ($query->num_rows() > 0) {
            ...
            $session_id = 1; // Default fallback
        }
        
        // Cache it in session
        $CI->session->set_userdata('set_session_id', $session_id);
    }
    return $session_id;
}
...
function get_global_setting($name = '')
{
    ...
    $name = trim($name);
    $CI->db->where('id', 1);
    $CI->db->select('global_settings.' . $name);
 ...
 */
function has_active_subscription($branch_id = null)
{
    $ci = &get_instance();
    ...
    $subscription = $ci->db->get('branch_subscriptions')->row();
    
    return !empty($subscription);
}

/**
 * Get branch's active plan
 */
function get_branch_plan($branch_id = null)
{
    $ci = &get_instance();
    ...
    $ci->db->select('bs.*, sp.name as plan_name');
    $ci->db->from('branch_subscriptions bs');
    $ci->db->join('subscription_plans sp', 'sp.id = bs.plan_id');
    $ci->db->where('bs.branch_id', $branch_id);
    $ci->db->where('bs.status', 'active');
    $ci->db->order_by('bs.id', 'DESC');
    $ci->db->limit(1);
    
    return $ci->db->get()->row();
}

// ==================== TEACHER INTELLIGENCE SYSTEM HELPERS ====================
...
if (!function_exists('get_current_term_id')) {
    function get_current_term_id() {
        ...
        // Get active term for this branch
        $term = $CI->db->select('id')
            ->where('is_active', 1)
...
if (!function_exists('get_teacher_workload_status')) {
    function get_teacher_workload_status($teacher_id, $session_id = null) {
        $CI =& get_instance();
        if (!$session_id) $session_id = get_session_id();
        
        $workload = $CI->db->select('status, workload_score')
        ...
        if (!$workload) {
            return ['status' => 'normal', 'score' => 0, 'badge_class' => 'success'];
        }
        
        $badge_class = 'success';
        if ($workload->status == 'overloaded') $badge_class = 'danger';
        elseif ($workload->status == 'underutilized') $badge_class = 'warning';
...
if (!function_exists('get_teacher_performance_rating')) {
    function get_teacher_performance_rating($teacher_id, $session_id = null, $term_id = null) {
        $CI =& get_instance();
        if (!$session_id) $session_id = get_session_id();
        if (!$term_id) $term_id = get_current_term_id();
        
        $score = $CI->db->select('total_score, rating, ranking')
            ->where('teacher_id', $teacher_id)
            ->where('session_id', $session_id)
        ...
        if (!$score) {
            return ['total_score' => 0, 'rating' => 'D', 'ranking' => 0, 'badge' => '<span class="badge badge-danger">D - Needs Improvement</span>'];
        }
        
        $badge = '<span class="badge badge-danger">D - Needs Improvement</span>';
        if ($score->rating == 'A') $badge = '<span class="badge badge-success">A - Excellent</span>';
        elseif ($score->rating == 'B') $badge = '<span class="badge badge-info">B - Good</span>';
        elseif ($score->rating == 'C') $badge = '<span class="badge badge-warning">C - Satisfactory</span>';
...
if (!function_exists('get_teacher_risk_level')) {
    function get_teacher_risk_level($teacher_id, $session_id = null, $term_id = null) {
        $CI =& get_instance();
        if (!$session_id) $session_id = get_session_id();
        if (!$term_id) $term_id = get_current_term_id();
        
        $risk = $CI->db->select('risk_level, prediction_score')
            ->where('teacher_id', $teacher_id)
            ->where('session_id', $session_id)
            ->where('term_id', $term_id)
...
if (!function_exists('get_overdue_duties_count')) {
    function get_overdue_duties_count($teacher_id = null, $branch_id = null) {
        $CI =& get_instance();
        if (!$branch_id) $branch_id = get_loggedin_branch_id();
        ...
        return $CI->db->get()->row()->count ?? 0;
    }
}

if (!function_exists('get_completed_duties_count')) {
    function get_completed_duties_count($teacher_id, $branch_id = null, $session_id = null) {
        $CI =& get_instance();
        if (!$branch_id) $branch_id = get_loggedin_branch_id();
        if (!$session_id) $session_id = get_session_id();
        
        return $CI->db->select('COUNT(*) as count')
            ->from('teacher_duties')
            ->where('teacher_id', $teacher_id)
            ->where('branch_id', $branch_id)
            ->where('session_id', $session_id)
...
if (!function_exists('get_teacher_workload_class')) {
    function get_teacher_workload_class($status) {
        switch($status) {
            case 'overloaded':
                return 'danger';
            case 'underutilized':
...
// ==================== TIS HELPER FUNCTIONS ====================


if (!function_exists('get_pending_duties_count')) {
    function get_pending_duties_count($teacher_id = null, $branch_id = null) {
        $CI =& get_instance();
        if (!$branch_id) $branch_id = get_loggedin_branch_id();
...
if (!function_exists('get_duty_status_class')) {
    function get_duty_status_class($status) {
        switch($status) {
