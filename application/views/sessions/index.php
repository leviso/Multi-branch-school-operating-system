					...
					<table class="table table-bordered table-hover table-condensed mb-md">
						...
						?>
								<tr>
									...
									<td>
										<!-- Set as active session button -->
										<?php if (get_session_id() != $row['id']) :?>
										<a href="<?php echo base_url('sessions/set_academic/'.$row['id']); ?>" 
										   class="btn btn-success btn-circle icon" 
										   data-toggle="tooltip" 
										   title="Set as Active Session"
										   onclick="return confirm('Are you sure you want to change the active session?');">
											<i class="fas fa-check-circle"></i>
										</a>
										<?php else: ?>
										<a href="javascript:void(0);" 
										   class="btn btn-primary btn-circle icon disabled" 
										   data-toggle="tooltip" 
										...
										<!-- Edit link -->
										<a class="btn btn-default btn-circle icon editModal" href="javascript:void(0);" 
										   data-id="<?=$row['id']?>" 
										   data-session="<?=$row['school_year']?>">
											<i class="fas fa-pen-nib"></i>
										</a>
										
										<!-- Delete link - Hide for active session -->
										<?php if (get_session_id() != $row['id']) :?>
											<?php echo btn_delete('sessions/delete/' . $row['id']); ?>
										<?php endif; ?>
									</td>
								</tr>
