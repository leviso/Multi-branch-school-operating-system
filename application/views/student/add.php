...
$is_stars_transfer = ($stars_source == 'stars_transfer');
?>
...
<section class="panel">
	...
	<input type="hidden" name="source" value="<?=$stars_source?>">
...
<?php } ?>	
	<div class="col-md-12">
		<section class="panel">
			...
			<input type="hidden" name="is_transfer_student" value="<?=$is_stars_transfer ? '1' : '0'?>">
			<header class="panel-heading">
				...
				<?php } ?>
				<h4 class="panel-title"><i class="fas fa-graduation-cap"></i> <?=translate('student_admission')?>
					<?php if ($is_stars_transfer): ?>
						<span class="label label-success ml-md"><i class="fas fa-exchange-alt"></i> Transfer Student</span>
					<?php endif; ?>
				</h4>
			</header>
			<div class="panel-body">
...
<div id="multipleImport" class="zoom-anim-dialog modal-block modal-block-lg mfp-hide">
    <section class="panel">
        ...
        <?php echo form_open_multipart('student/csv_import', array('class' => 'form-horizontal', 'id' => 'importCSV')); ?>
            <div class="panel-body">
                <div id="errorList" class="alert alert-danger" style="display: none;"></div>
                
                <div class="alert alert-subl mb-md">
                    <strong><?=translate('instructions')?> :</strong><br/>
                    1. <?=translate('download_sample_file_first')?><br/>
                    2. <?=translate('fill_student_details_carefully')?><br/>
                    3. <?=translate('date_format_instruction')?> (<?=date('Y-m-d')?>)<br/>
                    4. <?=translate('required_fields')?>: FirstName, RegisterNo, StudentUsername, StudentPassword, GuardianName, GuardianRelation, GuardianMobileNo, GuardianEmail
                </div>
                
                <div class="row mb-sm">
                    <div class="col-md-12 text-right">
                        <a href="<?=base_url('student/csv_Sampledownloader')?>" class="btn btn-default">
...
<?php } ?>
                <div class="form-group">
                    <label class="col-md-3 control-label"><?=translate('academic_year')?> <span class="required">*</span></label>
                    <div class="col-md-9">
                        <select name="year_id" class="form-control" id="academicYearModal">
                            <option value=""><?=translate('select')?></option>
                            <?php 
                            $years = $this->db->get('schoolyear')->result();
                            foreach ($years as $year){ ?>
                                <option value="<?=$year->id?>" <?=($year->id == get_session_id()) ? 'selected' : ''?>><?=$year->school_year?></option>
                            <?php } ?>
                        </select>
                        <span class="error"></span>
                    </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <?php
                            $arrayClass = $this->app_lib->getClass($branch_id);
                            echo form_dropdown("class_id", $arrayClass, set_value('class_id'), "class='form-control' id='classID_mod'");
                        ?>
                        <span class="error"></span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-md-9">
                        <select class="form-control" name="section_id" id="sectionID_mod">
                            <option value=""><?=translate('select')?></option>
                        </select>
                        <span class="error"></span>
                    </div>
                ...
                <div class="form-group mb-xs">
                    <label class="col-md-3 control-label"><?=translate('select_csv_file')?> <span class="required">*</span></label>
                    <div class="col-md-9">
            ...
            <footer class="panel-footer">
                <div class="row">
                    <div class="col-md-12 text-right">
                        ...
                        <button type="button" class="btn btn-default modal-dismiss" onclick="closeImportModal()"><?php echo translate('close'); ?></button>
                    </div>
                </div>
            </footer>
        <?php echo form_close(); ?>
...
<script type="text/javascript">
        ...
        $.magnificPopup.open({
            ...
            callbacks: {
                ...
                open: function() {
                    $('#importCSV')[0].reset();
                    $('#errorList').hide().html('');
                    $('#sectionID_mod').html('<option value=""><?=translate('select')?></option>');
                    $('.dropify').dropify();
                }
            }
        });
    ...
    function loadModalSections(classID) {
        if (classID != "") {
            $('#sectionID_mod').html('<option value=""><?=translate('loading')?>...</option>');
            $.ajax({
    ...
    $(document).ready(function() {
        // Handle class selection change for modal
        $(document).on('change', '#classID_mod', function() {
            var classID = $(this).val();
            loadModalSections(classID);
        });
        
        // Handle branch change for superadmin in modal
        $(document).on('change', '#branchID_mod', function() {
        ...
        $('#importCSV').on('submit', function(e) {
            ...
            $btn.html('<i class="fas fa-spinner fa-spin"></i> <?=translate('processing')?>...');
            $('#errorList').hide();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'success') {
                        $('#errorList').removeClass('alert-danger').addClass('alert-success').html('<i class="fas fa-check-circle"></i> ' + response.message).show();
                        setTimeout(function() {
                            closeImportModal();
                            location.reload();
                        }, 2000);
                    } else if (response.status == 'partial') {
                        $('#errorList').removeClass('alert-danger').addClass('alert-warning').html('<i class="fas fa-exclamation-triangle"></i> ' + response.message).show();
                    } else {
                        $('#errorList').removeClass('alert-success').addClass('alert-danger').html('<i class="fas fa-exclamation-circle"></i> ' + response.message).show();
                    }
                },
                error: function(xhr, status, error) {
                    $btn.prop('disabled', false);
                    $btn.html(originalText);
                    $('#errorList').removeClass('alert-success').addClass('alert-danger').html('<i class="fas fa-exclamation-circle"></i> Server error occurred. Please try again.').show();
                }
            });
        });

		// Preserve source parameter when branch changes
		$('#branch_selector').on('change', function() {
			var branchId = $(this).val();
			var source = '<?=$stars_source?>';
			if (source) {
				window.location.href = base_url + 'student/add?branch_id=' + branchId + '&source=' + source;
			}
		});
    });
</script>