		    ...
		    <div class="panel-body">
		        ...
		        <div class="mt-lg">
		        	...
		        	<ul class="stu-disabled">
						...
						<li>
						<a href="#hostel_items" data-toggle="tab">
							<i class="fas fa-boxes"></i> <?=translate('hostel_items')?>
						</a>
					</li>
		        	</ul>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="min-w-c">
										...
										<?php if (get_permission('stars', 'is_add')): ?>
										<a href="<?=base_url('stars/create_for_existing/' . $student['student_id'])?>" 
										class="btn btn-success"
										onclick="return confirm('Create a recovery assessment for this student? This will allow you to identify and address academic gaps.');">
											<i class="fas fa-chalkboard-user"></i> Start Recovery
										</a>
										<?php endif; ?>
                                    </td>
                                </tr>
...
<!-- Hostel Items Panel - ADD THIS NEW PANEL -->
<div class="panel panel-accordion">
    <div class="panel-heading">
        <h4 class="panel-title">
            <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#hostel_items">
                <i class="fas fa-boxes"></i> <?=translate('hostel_items')?>
            </a>
        </h4>
    </div>
    <div id="hostel_items" class="accordion-body collapse in">
        <div class="panel-body">
            <div class="table-responsive">
...
<script type="text/javascript">
	...
	// ========== HOSTEL ITEMS FUNCTIONS ==========
	$(document).ready(function() {
	    // Load hostel items when the accordion is clicked
	    $('a[href="#hostel_items"]').on('click', function() {
	        loadAssignedHostelItems();
	    });
	});
	
	function loadAssignedHostelItems() {
	    var student_id = <?php echo $student['id']; ?>;
	    
	    $('#hostel_items_tbody').html('<tr><td colspan="6" class="text-center"><i class="fas fa-spinner fa-spin"></i> Loading...<\/td><\/tr>');
	    
	    $.ajax({
	        url: base_url + 'student/get_assigned_hostel_items',
	        type: 'POST',
	        data: {
	            ...
	            if (response.length > 0) {
	                for (var i = 0; i < response.length; i++) {
	                    ...
	                    switch(item.condition) {
	                        ...
	                        case 'fair': conditionClass = 'warning'; break;
	                        case 'poor': conditionClass = 'danger'; break;
	                        default: conditionClass = 'default';
	                    }
	                    ...
	                    html += '<tr>' +
	                        ...
	                        '<td><span class="label label-' + conditionClass + '">' + (item.condition || 'N/A') + '<\/span><\/td>' +
	                        '<td>' + (item.assigned_date ? formatDate(item.assigned_date) : '-') + '<\/td>' +
	                        '<td><span class="label label-primary">Assigned<\/span><\/td>' +
	                        '<\/tr>';
	                }
	            } else {
	                html = '<tr><td colspan="6" class="text-center">No hostel items assigned to this student<\/td><\/tr>';
	            }
	            $('#hostel_items_tbody').html(html);
	        },
	        error: function(xhr, status, error) {
	            console.error('AJAX Error:', error);
	            $('#hostel_items_tbody').html('<tr><td colspan="6" class="text-center text-danger">Error loading items: ' + error + '<\/td><\/tr>');
	        }
	    });
	}
