...
<?php endif; ?>
							...
							<th><?php echo translate('returnable'); ?></th>
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo $row['status'] == 0 ? '<span class="label label-danger-custom">' . translate('not_returned') . '</span>' : _d($row['return_date']); ?></td>
							<td><?php echo html_escape(get_type_name_by_id('staff', $row['prepared_by'])); ?></td>
							<td>
								<a href="javascript:void(0);" class="btn btn-default btn-circle icon" data-toggle="tooltip" data-original-title="<?php echo translate('details'); ?>" onclick="getIssueDetails('<?php echo $row['id']; ?>')">
									<i class="fas fa-bars"></i>
								</a>
								<?php if (get_permission('product_issue', 'is_delete')): ?>
									<?php echo btn_delete('inventory/issue_delete/' . $row['id']); ?>
								<?php endif; ?>
							</td>
						</tr>
						<?php endforeach; 
						...
						<tr>
							<td colspan="<?php echo is_superadmin_loggedin() ? '10' : '9'; ?>" class="text-center"><?php echo translate('no_information_available'); ?></td>
						</tr>
						<?php endif; ?>
					</tbody>
				</table>
