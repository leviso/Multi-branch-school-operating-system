...
<?php endif; ?>
							...
							<td>
								...
								} elseif ($row['reorder_point'] > 0 && $row['available_stock'] <= $row['reorder_point']) {
									echo '<span class="label label-warning">' . translate('low_stock') . ' (' . $row['available_stock'] . ' ' . translate('left') . ')</span>';
								} else {
									echo '<span class="label label-success">' . translate('in_stock') . ' (' . $row['available_stock'] . ')</span>';
								}
								?>
							</td>
							<td class="min-w-xs">
		...
		<?php if (get_permission('product', 'is_add')){ ?>
			<div id="create" class="tab-pane">
				<?php echo form_open($this->uri->uri_string(), array('class' => 'form-horizontal form-bordered frm-submit')); ?>
					...
					<div class="form-group">
						<label class="col-md-3 control-label"><?=translate('reorder_point')?></label>
						<div class="col-md-6">
							<input type="number" class="form-control" name="reorder_point" value="<?php echo set_value('reorder_point', $product['reorder_point'] ?? 0); ?>" />
							<span class="help-block">Alert when stock falls below this number</span>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-3 control-label"><?=translate('returnable')?></label>
						<div class="col-md-6">
							<div class="checkbox-custom checkbox-default">
								<input type="checkbox" name="is_returnable" id="is_returnable" value="1" checked>
								<label for="is_returnable"><?=translate('allow_item_return')?></label>
							</div>
							<span class="help-block"><?=translate('uncheck_if_this_item_cannot_be_returned')?></span>
						</div>
					</div>
					<div class="form-group">
