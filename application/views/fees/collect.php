...
<section class="panel">
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="active">
				<div id="sms-status-container"></div>
				...
				<div id="collect_fees" class="tab-pane">
					<?php echo form_open('fees/fee_add', array('class' => 'form-horizontal frm-submit' )); ?>
						...
						<div class="form-group">
							...
							<div class="col-md-6">
								<?php
									// Get offline payment methods for this branch
									$payvia_list = array('' => translate('select_payment_method'));
									$offline_payments = $this->db->where('branch_id', $basic['branch_id'])->get('offline_payment_types')->result();
									foreach ($offline_payments as $payment) {
										$payvia_list[$payment->id] = $payment->name;
									}
						...
						<div class="form-group">
							...
							<div class="col-md-6 mb-md">
								...
								<!-- ========== FIXED: SMS SECTION WITH BALANCE DISPLAY ========== -->
								...
								<div class="mt-xs text-muted" id="sms-info-section">
									<small>
										<i class="fas fa-info-circle"></i> 
										SMS credits will be deducted from branch balance.
										Current balance: 
										<span id="sms-balance-indicator" class="text-primary">
											<?php 
											$ci =& get_instance();
											$ci->load->model('sendsmsmail_model');
											$sms_balance = $ci->sendsmsmail_model->get_sms_credit($basic['branch_id']);
											echo $sms_balance; 
											?> credits
										</span>
										<?php if ($sms_balance < 5): ?>
											<span id="sms-low-balance-warning" class="text-danger ml-2">
												<i class="fas fa-exclamation-triangle"></i> Low balance!
											</span>
										<?php endif; ?>
									</small>
								</div>
								<div id="sms-cost-estimate" class="mt-1"></div>
								<!-- ========== END SMS FIXES ========== -->
					...
					<?php echo form_open('fees/fee_fully_paid', array('class' => 'form-horizontal frm-submit' )); ?>
						...
						<div class="form-group">
							...
							<div class="col-md-6 mb-md">
								...
								<!-- ========== FIXED: SMS SECTION FOR FULLY PAID ========== -->
								<div class="checkbox-replace mt-lg">
									<label class="i-checks">
										<input type="checkbox" name="guardian_sms" checked> <i></i> Guardian Confirmation Sms
									</label>
...
<script type="text/javascript">
	...
	function updateSMSBalance() {
		$.ajax({
			...
			success: function (response) {
				if (response.success) {
					var balanceText = response.balance + ' credits';
					$('#sms-balance-indicator').text(balanceText);
					
					if (response.balance < 5) {
						if (!$('#sms-low-balance-warning').length) {
							$('#sms-balance-indicator').after(
								'<span id="sms-low-balance-warning" class="text-danger ml-2">' +
								'<i class="fas fa-exclamation-triangle"></i> Low balance!' +
								'</span>'
							);
						...
						$('#sms-low-balance-warning').remove();
					}
				}
			},
			error: function() {
				$('#sms-balance-indicator').text('Error loading');
			}
		});
	}
	
	// ========== FORM SUBMISSION HANDLER ==========
	function handleFeeFormSubmit(e, form) {
		e.preventDefault();
		...
		var formId = form.closest('.tab-pane').attr('id');
		
		form.find('.error').html('');
		$('#sms-status-container').html('');
		$('#sms-info-section').removeClass('text-danger');
		$('#sms-balance-indicator').removeClass('text-danger');
			...
			success: function(response) {
				...
				if (response.status === 'success') {
					...
					if (response.sms_status) {
						smsInfo = '\n\nSMS Status: ' + response.sms_status.message;
					}
					
					swal({
						title: "Success!",
						text: message + smsInfo,
						type: "success",
					...
					if (response.error) {
						$.each(response.error, function(key, value) {
							var errorElement = form.find('[name="' + key + '"]').parent();
							if (errorElement.length) {
								errorElement.find('.error').html('<span class="text-danger">' + value + '</span>');
							}
							
							if (key === 'guardian_sms') {
								$('#sms-info-section').addClass('text-danger');
								$('#sms-balance-indicator').addClass('text-danger');
	...
	function initializeFormHandlers() {
		$('#collect_fees .frm-submit').off('submit');
		$('#fully_paid .frm-submit').off('submit');
		
		$('#collect_fees .frm-submit').on('submit', function(e) {
			handleFeeFormSubmit(e, $(this));
		});
		
		$('#fully_paid .frm-submit').on('submit', function(e) {
			handleFeeFormSubmit(e, $(this));
		});
	...
	$('#collectFees').on('click', function(e) {
		...
		if (arrayData.length === 0) {
			swal({
				title: "No Selection",
				text: "Please select at least one fee item to collect.",
				type: "warning",
				buttonsStyling: false,
				confirmButtonClass: "btn btn-default swal2-btn-default",
				showCloseButton: true
	...
	$('#invoicePrint').on('click', function(e) {
		...
		if (arrayData.length === 0) {
			swal({
				title: "No Selection",
				text: "Please select at least one fee item to print.",
				type: "warning",
				buttonsStyling: false,
				confirmButtonClass: "btn btn-default swal2-btn-default",
				showCloseButton: true
	...
	$('#payReceiptPrint').on('click', function(e) {
		...
		if (arrayData.length === 0) {
			swal({
				title: "No Selection",
				text: "Please select at least one payment receipt to print.",
				type: "warning",
				buttonsStyling: false,
				confirmButtonClass: "btn btn-default swal2-btn-default",
				showCloseButton: true
	...
	$('#selected_revert').on('click', function(e){
		...
		if (paymentID.length === 0) {
			swal({
				title: "No Selection",
				text: "Please select at least one payment to revert.",
				type: "warning",
				buttonsStyling: false,
				confirmButtonClass: "btn btn-default swal2-btn-default",
				showCloseButton: true
	...
	// ========== SMS EVENT HANDLERS ==========
	$('input[name="guardian_sms"]').on('change', function() {
		if ($(this).is(':checked')) {
			updateSMSBalance();
		}
	});
	...
	$(document).ready(function() {
		...
		if ($('input[name="guardian_sms"]').is(':checked')) {
			updateSMSBalance();
		}
		
		$('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
			var targetTab = $(e.target).attr('href');
			if (targetTab === '#collect_fees' || targetTab === '#fully_paid') {
				updateSMSBalance();
			}
			$('#sms-status-container').html('');
		});
		...
		$('#collect_fees input[name="amount"], #collect_fees input[name="discount_amount"], #collect_fees input[name="fine_amount"]').on('change', function() {
			updateSMSBalance();
		});
		
		$(document).on('closed', '.mfp-close, .modal-dismiss', function() {
			setTimeout(function() {
				initializeFormHandlers();
			}, 100);
		});
		
		console.log('Document ready - M-Pesa STK initialized');
	});
	
	// ========== HELPER FUNCTIONS ==========
	function showSMSWarning(message, isError) {
		var alertClass = isError ? 'alert-danger' : 'alert-warning';
		var icon = isError ? 'fa-exclamation-triangle' : 'fa-info-circle';
		...
		setTimeout(function() {
			$('#sms-status-container .alert').alert('close');
		}, 10000);
	}
	
	function debugFormHandlers() {
		console.log('=== Form Handler Debug ===');
		console.log('isSubmitting:', isSubmitting);
		console.log('Collect Fees form handlers:', $('#collect_fees .frm-submit').data('events'));
		console.log('Fully Paid form handlers:', $('#fully_paid .frm-submit').data('events'));
