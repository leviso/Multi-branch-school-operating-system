						...
						<div class="panel-group" id="accordion">
							...
							if (count($templatelist)){
								foreach ($templatelist as $template):
									...
									// Create safe ID using template ID instead of name
									$template_safe_id = 'template_' . $template['id'];
									?>		
								<div class="panel panel-accordion">
									<div class="panel-heading">
										<h4 class="panel-title">
											<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#<?=$template_safe_id?>">
												<i class="far fa-comments"></i> <?=translate($template['name'])?>
											</a>
										</h4>
									</div>
									<div id="<?=$template_safe_id?>" class="accordion-body collapse">
												...
												<div class="row">
														...
														<div class="form-group">
															...
															<div class="checkbox-replace">
																<label class="i-checks">
																	<input type="checkbox" name="notify_student" id="notify_student_<?=$template['id']?>" <?=($getRow['notify_student'] == 1 ? 'checked' : '');?>>
																	<i></i> <?=translate('student')?>
																</label>
															</div>
															<div class="checkbox-replace mt-xs">
																<label class="i-checks">
																	<input type="checkbox" name="notify_parent" id="notify_parent_<?=$template['id']?>" <?=($getRow['notify_parent'] == 1 ? 'checked' : '');?>>
													...
													<?php } ?>
														...
														<div class="form-group">
															...
															<textarea class="form-control message-text" name="template_body" rows="3" data-id="<?=$template['id']?>"><?=$getRow['template_body']?></textarea>
															<span class="error"></span>
															<span class="help-block txtcount pull-right" id="count_<?=$template['id']?>">You typed <?=strlen($getRow['template_body'])?> characters.</span>
														</div>
														<div class="md">
...
<script type="text/javascript">
	$(document).ready(function () {
		$(document).on('keyup', '.message-text', function() {
			...
			$('#count_' + id).html('You typed ' + text_length + ' characters.');
		});
	});
</script>