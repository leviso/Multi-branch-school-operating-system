				...
				<table class="table table-bordered table-hover table-condensed mb-none tbr-top table-export">
					...
					<tbody>
						...
						<tr>
							...
							<td class="text-center">
								<img src="<?php echo $this->application_model->get_book_cover_image($row['cover']); ?>" alt="" width="70">
							</td>
							...
							<td class="text-center"><?php echo _d($row['date_of_issue']); ?></td>
							<td class="text-center"><?php echo _d($row['date_of_expiry']); ?></td>
							<td class="text-right"><?php echo $global_config['currency_symbol'] . $row['fine_amount']; ?></td>
							<td class="text-center">
								<?php
								$status = $row['status'];
								if($status == 0)
		...
		<?php if (get_permission('book_manage', 'is_add')): ?>
			<div class="tab-pane" id="issue">
				<?php echo form_open('library/bookIssued', array('class' => 'form-horizontal form-bordered frm-submit')); ?>
					...
					<!-- Barcode Scanner Section - MOVED OUTSIDE SUPERADMIN CONDITION -->
					<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title"><i class="fas fa-qrcode"></i> <?=translate('barcode_scanner')?></h4>
						</div>
						<div class="panel-body">
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										<label class="control-label"><?=translate('scan_barcode')?></label>
										<div class="input-group">
											<input type="text" class="form-control" id="barcode_input" placeholder="<?=translate('scan_or_enter_barcode')?>" autofocus>
											<span class="input-group-btn">
												<button class="btn btn-primary" id="scan_barcode_btn" type="button">
													<i class="fas fa-search"></i> <?=translate('find_book')?>
												</button>
											</span>
										</div>
										<small class="text-muted"><?=translate('use_barcode_scanner_or_manually_enter_code')?></small>
									</div>
								</div>
								<div class="col-md-6" id="barcode_result" style="display: none;">
									<div class="alert alert-info">
										<strong id="barcode_book_title"></strong><br>
										<span id="barcode_book_details"></span>
									</div>
								</div>
							</div>
						</div>
					...
					<!-- Branch field for Superadmin only -->
					...
					<!-- Rest of the form fields (category, book_title, role, etc.) -->
					<div class="form-group">
...
<div class="zoom-anim-dialog modal-block modal-block-primary mfp-hide" id="bookReturn">
	<section class="panel">
		...
		<input type="hidden" name="scanned_copy_number" id="scanned_copy_number" value="">
			<div class="panel-body">
...
function checkUserBorrowingLimit() {
    ...
    $.ajax({
        url: base_url + 'library/check_borrowing_limit_ajax',
        type: 'POST',
        data: {
            user_id: user_id,
            role_id: role_id,
        ...
        success: function(response) {
            ...
            $('.borrowing-limit-warning').remove();
            
            if (!response.can_borrow) {
                var warning = $('<div class="alert alert-warning borrowing-limit-warning mt-sm">' +
                    '<i class="fas fa-exclamation-triangle"></i> ' +
                    'Borrowing limit reached! This user already has <strong>' + response.current_borrowed + '</strong> out of <strong>' + response.limit + '</strong> books. ' +
                    'Please ask them to return some books first.' +
                    '</div>');
                $('#user_id').closest('.form-group').after(warning);
                $('#book_issue_submit').prop('disabled', true);
            } else {
                var info = $('<div class="alert alert-info borrowing-limit-warning mt-sm">' +
                    '<i class="fas fa-info-circle"></i> ' +
                    'This user has borrowed <strong>' + response.current_borrowed + '</strong> out of <strong>' + response.limit + '</strong> books. ' +
                    '<strong>' + response.remaining + '</strong> more book(s) can be issued.' +
                    '</div>');
                $('#user_id').closest('.form-group').after(info);
                $('#book_issue_submit').prop('disabled', false);
            }
        }
    });
}
...
var scannedBarcodeValue = '';  // Store the scanned barcode

function scanBarcode() {
    var barcode = $('#barcode_input').val().trim();
    if (!barcode) {
        alert('<?=translate('please_enter_barcode')?>');
        return;
    }
    ...
    $('#scanned_copy_number').val(barcode);
    
    var $btn = $('#scan_barcode_btn');
    $btn.html('<i class="fas fa-spinner fa-spin"></i>').prop('disabled', true);
    $('#barcode_result').hide();
    
    $.ajax({
        url: base_url + 'library/get_book_by_barcode',
        type: 'POST',
        ...
        success: function(response) {
            if (response.status == 'success') {
                ...
                $('#barcode_result').show();
                $('#barcode_book_title').text(book.title);
                $('#barcode_book_details').html(
                    '<?=translate('author')?>: ' + book.author + '<br>' +
                    '<?=translate('copy_number')?>: <strong>' + (book.copy_number || barcode) + '</strong><br>' +
                    '<?=translate('status')?>: ' + (book.is_available ? 'Available' : 'Not Available')
                );
                
                if (book.is_available && book.category_id) {
                    $('#category_id').val(book.category_id);
                    $('#category_id').trigger('change');
                    
                    setTimeout(function() {
                        if ($('#book_id option[value="' + book.book_id + '"]').length) {
                            $('#book_id').val(book.book_id);
                            $('#book_id').trigger('change');
                            alert('<?=translate('book_found')?> - Ready to issue copy: ' + (book.copy_number || barcode));
                        } else {
                            alert('<?=translate('book_found_but_not_in_list')?>');
                        }
                    }, 1000);
                } else if (book.is_available) {
                    alert('<?=translate('book_found')?>');
                } else {
                    alert('<?=translate('book_not_available')?>');
                }
            } else {
        ...
        error: function(xhr, status, error) {
            console.error("Barcode Scan Error:", xhr.responseText);
            alert('<?=translate('error_scanning_barcode')?>');
        },
        complete: function() {
            $btn.html('<i class="fas fa-search"></i> <?=translate('find_book')?>').prop('disabled', false);
...
$(document).ready(function() {
    // Add hidden fields to store scanned barcode
    $('form.frm-submit').append('<input type="hidden" name="scanned_barcode" id="scanned_barcode" value="">');
    $('form.frm-submit').append('<input type="hidden" name="scanned_copy_number" id="scanned_copy_number" value="">');
});

// Add hidden field to form to submit scanned barcode
$(document).ready(function() {
    // Add hidden input to the issue form
    $('form.frm-submit').append('<input type="hidden" name="scanned_barcode" id="scanned_barcode" value="">');
    
    // Update hidden field when barcode is scanned
    $('#scan_barcode_btn, #barcode_input').on('click keypress', function() {
        setTimeout(function() {
            $('#scanned_barcode').val(scannedBarcode);
        }, 500);
    });
});
</script>