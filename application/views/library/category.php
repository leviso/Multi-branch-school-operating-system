						...
						<tbody>
							...
							<tr>
								...
								<?php if (get_permission('book_category', 'is_edit')): ?>
									<!-- update link -->
									<a class="btn btn-default btn-circle icon" onclick="getCategoryModal(this)" 
										data-id="<?=$row['id']?>" 
										data-name="<?=$row['name']?>" 
...
function getCategoryModal(btn) {
    var id = $(btn).data('id');
    var name = $(btn).data('name');
    var branch_id = $(btn).data('branch');
    ...
    <?php if (is_superadmin_loggedin()): ?>
    if (branch_id) {
        $('#ebranch_id').val(branch_id).trigger('change');
    }
    <?php endif; ?>
